/******/ (() => { // webpackBootstrap
  ...
  (function ($) {
    ...
    $(document).ready(function () {
      ...
      $('#input-search-slider').on('keydown', function () {
        $('.ps-search--mobile .ps-panel--search-result').html('').removeClass('active');
      }).on('keyup', function () {
        var k = $(this).val();
  
        if (k) {
          $('#search-sidebar .spinner-icon').show();
          clearTimeout(searchTimeout);
          searchTimeout = setTimeout(function () {
            var form = $('#search-sidebar .ps-form--search-mobile');
            $.ajax({
              type: 'GET',
              url: form.data('ajax-url'),
              data: form.serialize(),
              success: function success(res) {
                if (!res.error && res.data !== '') {
                  $('#search-sidebar .ps-panel--search-result').html(res.data).addClass('active');
                } else {
                  $('#search-sidebar .ps-panel--search-result').html('').removeClass('active');
                }
  
                $('#search-sidebar .spinner-icon').hide();
              },
              error: function error() {
                $('#search-sidebar .spinner-icon').hide();
              }
      ...
      $('#input-search-m').on('keydown', function () {
        $('.ps-search--mobile .ps-panel--search-result').html('').removeClass('active');
      }).on('keyup', function () {
        var k = $(this).val();
        ...
        if (k) {
          ...
          searchTimeout = setTimeout(function () {
            ...
            $.ajax({
              ...
              success: function success(res) {
                if (!res.error && res.data !== '') {
                  $('.ps-search--mobile .ps-panel--search-result').html(res.data).addClass('active');
                } else {
                  $('.ps-search--mobile .ps-panel--search-result').html('').removeClass('active');
                }
  
                $('.ps-search--mobile .spinner-icon').hide();
              },
              error: function error() {
                $('.ps-search--mobile .spinner-icon').hide();
              }
    ...
    $("#verifyOTP").click(function(){
      const post_phoneNumber = $("[name=phone]").val();
